...
def read_mat_argo(filepath, pa_float_name):
    ...
    not_use = []
    for i in range(data["lat"].__len__()):
...
def get_topo_grid(min_long, max_long, min_lat, max_lat, config):
    ...
    elev_file = open(config["ELEVATION_FILE"], "rb")  # pylint: disable=consider-using-with
...
def get_region_data(pa_wmo_numbers, pa_float_name, config, index, pa_float_pres):
    ...
    max_pres = np.nanmax(pa_float_pres) + int(config["MAP_P_DELTA"])
                    ...
                    for i in range(data_length):
                        ...
                        if good_indices.__len__() > 0:
                            ...
                            if new_depth > max_depth != 0:
                                grid_pres = np.append(
                                    grid_pres,
                                    np.ones((how_many_cols, new_depth - max_depth)) * np.nan,
                                    axis=1,
...
def frontal_constraint_saf(config, grid_data, float_data):
    ...
    profile_around_saf_path = os.path.join(config["CONFIG_DIRECTORY"], config["CONFIG_SAF"])
    typical_profile_around_saf = loadmat(profile_around_saf_path)
    ...
    if float_lat < -30:
        ...
        if (
            ...
            if crit_saf == 0:
                sal_int = np.interp(
                    deph.flatten(),
                    float_pres[isok].flatten(),
                    float_sal[isok].flatten(),
                    left=np.nan,
                    right=np.nan,
                )
                temp_int = np.interp(
