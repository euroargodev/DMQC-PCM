...
def update_salinity_mapping(float_name, config, pcm_classes_dir):
    ...
    float_source_data_path = os.path.join(config['FLOAT_SOURCE_DIRECTORY'],
    ...
    wmo_boxes_path = os.path.join(config['CONFIG_DIRECTORY'],
    ...
    mapped_data_path = os.path.join(config['FLOAT_MAPPED_DIRECTORY'],
        ...
        if not np.isnan(float_data['float_long']) and not np.isnan(float_data['float_lat']) \
            ...
            if grid_lat.__len__() > 0:
                ...
                index = find_besthist(grid_data['grid_lat'], grid_data['grid_long'], grid_data['grid_dates'],
                                      ...
                                      map_use_pv, max_casts, use_pcm, pcm_file_path)
    ...
    save_location = os.path.join(config['FLOAT_MAPPED_DIRECTORY'],
...
def calc_piecewisefit(float_name, system_config):
    ...
    # load in the source data
    float_source_data_path = os.path.join(system_config['FLOAT_SOURCE_DIRECTORY'],
    ...
    # load in the mapped data
    float_mapped_data_path = os.path.join(system_config['FLOAT_MAPPED_DIRECTORY'],
    ...
    float_calseries_path = os.path.join(system_config['FLOAT_CALIB_DIRECTORY'],
                                        system_config['FLOAT_CALSERIES_PREFIX'] + \
                                        float_name + system_config['FLOAT_CALIB_POSTFIX'])
    ...
    if sstatus == 0:
        ...
        float_calib_filename = os.path.join(system_config['FLOAT_CALIB_DIRECTORY'],
        ...
        if pp.__len__() > 0:
            ...
            if breaks.__len__() == 0:
                ...
                (xfit, condslope, condslope_err,
                 ...
                 fit_coef, fit_breaks, fit_type) = fit_cond(x, y, err,
                                                  covariance,
                                                  'max_no_breaks',
                                                  max_breaks[i][0])
    ...
    float_calib_name = os.path.join(system_config['FLOAT_CALIB_DIRECTORY'],
                                    system_config['FLOAT_CALIB_PREFIX'] + float_name +
                                    system_config['FLOAT_CALIB_POSTFIX'])
