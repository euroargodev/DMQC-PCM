...
def update_salinity_mapping(float_name, config, pcm_classes_dir):
    ...
    float_source_data_path = os.path.join(config["FLOAT_SOURCE_DIRECTORY"], float_name + config["FLOAT_SOURCE_POSTFIX"])
    ...
    wmo_boxes_path = os.path.join(config["CONFIG_DIRECTORY"], config["CONFIG_WMO_BOXES"])

    wmo_boxes = loadmat(wmo_boxes_path)

    max_casts = int(config["CONFIG_MAX_CASTS"])
    map_use_pv = int(config["MAP_USE_PV"])
    map_use_saf = int(config["MAP_USE_SAF"])
    long_large = float(config["MAPSCALE_LONGITUDE_LARGE"])
    long_small = float(config["MAPSCALE_LONGITUDE_SMALL"])
    lat_large = float(config["MAPSCALE_LATITUDE_LARGE"])
    lat_small = float(config["MAPSCALE_LATITUDE_SMALL"])
    phi_large = float(config["MAPSCALE_PHI_LARGE"])
    phi_small = float(config["MAPSCALE_PHI_SMALL"])
    map_age_large = int(config["MAPSCALE_AGE_LARGE"])
    map_age_small = int(config["MAPSCALE_AGE_SMALL"])
    map_p_delta = int(config["MAP_P_DELTA"])
    map_p_exclude = int(config["MAP_P_EXCLUDE"])

    use_pcm = int(config["USE_PCM"])
    ...
    for i in range(profile_no.__len__()):
    ...
    for i in range(missing_profile_index.__len__()):
        # start the timer
        ...
        if (
            ...
            x_axis = float_x[0, :]
            y_axis = float_y[:, 0]
            float_interp = RegularGridInterpolator((y_axis, x_axis), float_elev, method="linear")
            # Since you initialized with (y_axis, x_axis), we must pass (lat, long)
            points = (float_data["float_lat"], float_long_tbase)

            # float_data["float_z"] = -float_interp(float_long_tbase, float_data["float_lat"])[0]
            float_data["float_z"] = -float_interp(points)
    ...
    data = sort_numpy_array(
        data,
        sorted_profile_index,
        ["la_ptmp", "la_mapped_sal", "la_map_sal_errors", "la_noise_sal", "la_signal_sal"],
    )
...
# pylint: disable=too-many-branches
# pylint: disable=too-many-statements
def calc_piecewisefit(float_name, system_config):
    ...
    float_source_data_path = os.path.join(
        system_config["FLOAT_SOURCE_DIRECTORY"],
    ...
    # load in the mapped data
    float_mapped_data_path = os.path.join(
    ...
    if sstatus == 0:
        float_calib_filename = os.path.join(
            system_config["FLOAT_CALIB_DIRECTORY"],
        ...
        if pp.__len__() > 0:
            ...
            if pcond_factor[0][calindex].__len__() > 0:
                ...
                cal_sal1[:, calindex] = gsw.conversions.SP_from_C(
                    (cal_cond[:, calindex] + cal_cond_err[:, calindex]),
                    unique_ptmp,
                    0,
                )
