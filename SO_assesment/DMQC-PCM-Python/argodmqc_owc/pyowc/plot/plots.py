...
def trajectory_plot(bath, reef, floats, climatology, float_name, config):
    ...
    coastline = os.path.join(config["CONFIG_DIRECTORY"], "coastline", "ne_10m_coastline.shp")
    ...
    if reef:
        reef = os.path.join(config["CONFIG_DIRECTORY"], "reefs", "ne_10m_reefs.shp")
    ...
    plot_loc = os.path.join(config["FLOAT_PLOTS_DIRECTORY"], float_name)
    os.makedirs(plot_loc, exist_ok=True)

    # Build full filepath
    filepath = pathlibpath(f"{plot_loc}/{float_name}_1_trajectory.{save_format}")
    plt.savefig(filepath, format=save_format)
...
def theta_sal_plot(
    ...
    save_format = config["FLOAT_PLOTS_FORMAT"]
    plot_loc = os.path.join(config["FLOAT_PLOTS_DIRECTORY"], float_name)
    filepath = pathlibpath(f"{plot_loc}/{float_name}_2_{title}_theta_sal.{save_format}")
    plt.savefig(filepath, format=save_format)
    plt.show()
...
def t_s_profile_plot(sal, ptmp, pres, sal_var, theta_levels, tlevels, plevels, float_name, config):
    ...
    save_format = config["FLOAT_PLOTS_FORMAT"]
    plot_loc = os.path.join(config["FLOAT_PLOTS_DIRECTORY"], float_name)
    filepath = pathlibpath(f"{plot_loc}/{float_name}_3_salinity_profile.{save_format}")
    plt.savefig(filepath, format=save_format, bbox_inches="tight")
...
def sal_var_plot(
    ...
    for i in range(levels):
        for j in range(no_profiles):
            ...
            if ~np.isnan(theta_index):
                ...
                ptmp1 = float(ptmp[theta_index, j])
                ptmp2 = float(ptmp[k_index, j])

                # ---- sal interpolation ----
                sal1 = float(sal[theta_index, j])
                sal2 = float(sal[k_index, j])
                ...
                if (
                    ...
                    if ptmp1 != ptmp2:
                        ...
                        s_int[i, j] = sal1 + (sal2 - sal1) * (t - ptmp1) / (ptmp2 - ptmp1)
                    else:
                        s_int[i, j] = sal1
                else:
                    s_int[i, j] = sal1

                # ---- map_sal and map_sal_errors interpolation ----
                map1 = float(map_sal[theta_index, j])
                map2 = float(map_sal[k_index, j])
                map_err1 = float(map_sal_errors[theta_index, j])
                map_err2 = float(map_sal_errors[k_index, j])

                if (
                    k_index != theta_index
                    and not np.isnan(ptmp1)
                    and not np.isnan(ptmp2)
                    and not np.isnan(map1)
                    and not np.isnan(map2)
                ):
                    t = float(tlevels[i][0])
                    if ptmp1 != ptmp2:
                        s_map[i, j] = map1 + (map2 - map1) * (t - ptmp1) / (ptmp2 - ptmp1)
                        s_map_err[i, j] = map_err1 + (map_err2 - map_err1) * (t - ptmp1) / (ptmp2 - ptmp1)
                    else:
                        s_map[i, j] = map1
                        s_map_err[i, j] = map_err1
                else:
                    s_map[i, j] = map1
                    s_map_err[i, j] = map_err1

                # ---- cal_sal and cal_sal_errors interpolation ----
                cal1 = float(cal_sal[theta_index, j])
                cal2 = float(cal_sal[k_index, j])
                cal_err1 = float(cal_sal_errors[theta_index, j])
                cal_err2 = float(cal_sal_errors[k_index, j])

                if (
                    k_index != theta_index
                    and not np.isnan(ptmp1)
                    and not np.isnan(ptmp2)
                    and not np.isnan(cal1)
                    and not np.isnan(cal2)
                ):
                    t = float(tlevels[i][0])
                    if ptmp1 != ptmp2:
                        s_cal[i, j] = cal1 + (cal2 - cal1) * (t - ptmp1) / (ptmp2 - ptmp1)
                        s_cal_err[i, j] = cal_err1 + (cal_err2 - cal_err1) * (t - ptmp1) / (ptmp2 - ptmp1)
                    else:
                        s_cal[i, j] = cal1
                        s_cal_err[i, j] = cal_err1
                else:
                    s_cal[i, j] = cal1
                    s_cal_err[i, j] = cal_err1
    ...
    for i in range(levels):
        ...
        save_format = config["FLOAT_PLOTS_FORMAT"]
        plot_loc = os.path.join(config["FLOAT_PLOTS_DIRECTORY"], float_name)
        filepath = pathlibpath(f"{plot_loc}/{float_name}_4_salinity_variance_.{i + 1}.{save_format}")
        plt.savefig(filepath, format=save_format, bbox_inches="tight")
...
def cal_sal_curve_plot(
    ...
    if np.argwhere(~np.isnan(cal_sal)).__len__() > 0:
        ...
        save_format = config["FLOAT_PLOTS_FORMAT"]
        plot_loc = os.path.join(config["FLOAT_PLOTS_DIRECTORY"], float_name)
        filepath = pathlibpath(f"{plot_loc}/{float_name}_5_salinity_curve.{save_format}")
        plt.savefig(filepath, format=save_format, bbox_inches="tight")
        plt.show()
...
def sal_anom_plot(sal, ptmp, profile_no, config, float_name, title="uncalibrated"):
    ...
    for k in range(int(prof_range[0]), int(prof_range[1] + 1)):
        ...
        if good_data.__len__() >= 3:
            sal_int_interp = interpolate.interp1d(
                good_temp[good_data].flatten(),
                good_sal[good_data].flatten(),
                bounds_error=False,
            )
    ...
    for bounds in ast.literal_eval(config["THETA_BOUNDS"]):
        plt.figure(figsize=(10, 6))
        ...
        save_format = config["FLOAT_PLOTS_FORMAT"]
        plot_loc = os.path.join(config["FLOAT_PLOTS_DIRECTORY"], float_name)
        filepath = pathlibpath(f"{plot_loc}/{float_name}_6_{title}_salinity_anomaly_{bounds}.{save_format}")
        plt.savefig(filepath, format=save_format, bbox_inches="tight")
        plt.show()
