...
from ..plot.plots import theta_sal_plot, cal_sal_curve_plot, sal_var_plot,\
    t_s_profile_plot, trajectory_plot, sal_anom_plot
from ..plot.utils import create_dataframe
from ..core.finders import find_10thetas


#pylint: disable=too-many-locals
def plot_diagnostics(float_name, config, levels=3):
    ...
    grid_data_loc = os.path.join(config['FLOAT_MAPPED_DIRECTORY'],
                                 config['FLOAT_MAPPED_PREFIX'] + float_name +
                                 config['FLOAT_MAPPED_POSTFIX'])

    float_data_loc = os.path.join(config['FLOAT_SOURCE_DIRECTORY'],
                                  float_name + config['FLOAT_SOURCE_POSTFIX'])

    cal_data_loc = os.path.join(config['FLOAT_CALIB_DIRECTORY'],
                                config['FLOAT_CALIB_PREFIX'] + float_name +
                                config['FLOAT_CALIB_POSTFIX'])

    cal_series_loc = os.path.join(config['FLOAT_CALIB_DIRECTORY'],
                                  config['FLOAT_CALSERIES_PREFIX'] + float_name +
                                  config['FLOAT_CALIB_POSTFIX'])
    ...
    trajectory_plot(1, 1, floats, grid, float_name, config)
    plt.show()
