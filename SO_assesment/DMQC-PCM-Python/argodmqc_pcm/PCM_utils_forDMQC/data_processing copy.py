...
def interpolate_standard_levels(ds, std_lev):
    ...
    # compute a mask and apply to that rather than whole datset
    #i1 = (ds["pres"].max("n_pres") >= std_lev[-1]).compute()
    #ds = ds.isel(n_profiles=i1)
    ...
    # vars to interpolate
    datavars = [
        dv for dv in list(ds.variables) if set(["n_pres", "n_profiles"]) == set(ds[dv].dims) and "ptmp" not in dv
    ]
    # coords
    ...
    solovars = [
        ...
        if dv not in datavars and dv not in coords and "QC" not in dv and "ERROR" not in dv
    ]

    print((">>> datavars to interpolate: %s", datavars))
    print((">>> n_profiles: %d, n_pres: %d, std_lev: %d", len(ds.n_profiles), len(ds.n_pres), len(std_lev)))

    pres = ds.pres.load()
    for dv in datavars:
        print((">>> interpolating variable: %s", dv))
        var = ds[dv].load()
        ...
        print(f"\n>>> Debug for variable: {dv}")
        print(f"pres shape: {pres.shape}, dims: {pres.dims}")
        print(f"var shape: {var.shape}, dims: {var.dims}")
        print(f"n_pres coord length: {len(pres.n_pres)}")
        print(f"n_profiles coord length: {len(pres.n_profiles)}")
        # ds_out[dv] = interpolate_standard_levels_vectorized(
        #    pres,
        #    var,
        #    std_lev=std_lev,
        # )
    # ds_out = ds_out.rename({"remapped": "PRES_INTERPOLATED"})
    ds_out["PRES_INTERPOLATED"] = xr.DataArray(
        -np.abs(std_lev),
        dims=["z_levels"],
        coords={"z_levels": std_lev},
    )
    ...
    ds_out["PRES_INTERPOLATED"] = -np.abs(ds_out["PRES_INTERPOLATED"])
...
def linear_interpolation_remap(
    ...
    def _regular_interp(x, y, target_values):
        ...
        if len(y) < 5:
            ...
            # using np.interpolate to increase speed
            # interpolated = np.interp(
            #    target_values,  # x-coordinates where to interpolate
            #    x,  # x-coordinates of data points
            #    y,  # y-coordinates of data points
            #    left=y[0],  # value for target < x[0]
            #    right=y[-1],  # value for target > x[-1]
            # )
        return interpolated
    ...
    kwargs = dict(
        ...
        vectorize=False,  # Since _regular_interp already works on 1D arrays, you don't need vectorization.
        dask="parallelized",
...
# Numba compiles your Python loop into machine code, making that for i in range(n_profiles) loop run at C-speed.
@numba.njit(parallel=True)
def fast_interp(pres_np, var_np, std_np, out):
    n_profiles = pres_np.shape[0]
    for i in numba.prange(n_profiles):
        ...
        if len(x_m) >= 5:
            # Note: numba supports np.interp
            out[i] = np.interp(std_np, x_m, y_m)


def interpolate_standard_levels_vectorized(pres: xr.DataArray, var: xr.DataArray, std_lev) -> xr.DataArray:
    """Vectorized interpolation .

    Parameters
    ----------
    pres : xr.DataArray
        Shape: (n_profiles, n_pres)
    var : xr.DataArray
        Shape: (n_profiles, n_pres)
    std_lev : array-like
        Standard levels to interpolate to, shape (n_std_levels,)

    Returns
    -------
    xr.DataArray
        Interpolated variable, shape: (n_profiles, n_std_levels)

    """
    # Convert to numpy
    pres_np = pres.values.T  # (n_profiles, n_pres)
    var_np = var.values.T
    std_np = np.array(std_lev, dtype=np.float32)

    n_profiles, n_std = pres_np.shape[0], len(std_np)

    # Replace NaNs in pres with +inf to mask
    pres_masked = np.where(np.isnan(pres_np), np.inf, pres_np)

    # Prepare output
    out = np.full((n_profiles, n_std), np.nan, dtype=var_np.dtype)

    # Loop over profiles (fast because all in NumPy)
    fast_interp(pres_np, var_np, std_np, out)

    # Wrap as DataArray
    return xr.DataArray(
        out,
        dims=["n_profiles", "z_levels"],
        coords={"n_profiles": pres.n_profiles, "z_levels": std_lev},
        attrs=var.attrs,
    )
