...
import scipy as sp
import xarray as xr
from argodmqc_pcm.PCM_utils_forDMQC.config_context import get_current_config
from scipy.interpolate import RegularGridInterpolator


def make_safe_rgi(grid_x, grid_y, grid_elev, method="linear"):
    """Create a robust RegularGridInterpolator from meshgrid-style inputs.

    Parameters
    ----------
    grid_x : 2D or 1D array
        X coordinates (usually longitude)
    grid_y : 2D or 1D array
        Y coordinates (usually latitude)
    grid_elev : 2D array
        Elevation or values grid
    method : str
    ...
    """
    # --- 1️⃣ Extract 1D axes from meshgrid ---
    x = grid_x[0, :] if grid_x.ndim == 2 else grid_x
    y = grid_y[:, 0] if grid_y.ndim == 2 else grid_y

    x = np.asarray(x)
    y = np.asarray(y)
    values = np.asarray(grid_elev)

    # --- 2️⃣ Check shape consistency ---
    if values.shape == (len(y), len(x)):
        pass  # correct orientation

    elif values.shape == (len(x), len(y)):
        # Likely MATLAB-style ordering — transpose
        values = values.T
    ...
    # --- 3️⃣ Final safety assertion ---
    assert values.shape == (len(y), len(x)), "Internal error: grid still inconsistent after fix."

    # --- 4️⃣ Build interpolator (y first, x second) ---
...
def get_refdata(float_mat_path, config, map_pv_use=0):
    ...
    Updates:
    kamwal, 2022Jan02, updates of directories to the reference data and tbase decoder
    Clive Neil, 2022Jan, remove duplicates from list and sort into ascending order
    ...
    WMOboxes_latlon = np.loadtxt(config["WMO_BOXES_LAT_LONG"], skiprows=1)

    # select boxes
    boo_array = (
        (WMOboxes_latlon[:, 1] >= geo_extent[0] - 9)
    ...
    # boxes_list = sorted(list(set(boxes_list))) #kamwal commented 2022/03/09
    ...
    if argo_data & ctd_data:
        ...
        file_str = "argo_"
        folder = config["HISTORICAL_ARGO_PREFIX"]
    elif argo_data:
        file_str = "argo_"
        folder = config["HISTORICAL_ARGO_PREFIX"]
    elif ctd_data:
        file_str = "ctd_"
        folder = config["HISTORICAL_CTD_PREFIX"]
    ...
    for ifile in boxes_list:
        reference_file_path = ref_path + folder + str(int(ifile)) + ".mat"
        print(f">>> {reference_file_path}")

        try:
            mat_dict_load = sp.io.loadmat(reference_file_path)
        ...
        if cnt == 0:
            ...
            if pres_levels_load > pres_levels:
                ...
                n_prof_source = len(mat_dict_load["source"])

                if n_prof_source != n_prof:
                    print(f"XXX skipping box: {n_prof_source}, {n_prof}")
                    continue
...
def add_floatdata(float_WMO, float_mat_path, ds):
    ...
    source_matrix = [
        (source_matrix[i] + "_" + str(np.squeeze(mat_dict_float["PROFILE_NO"])[i])) for i in range(len(source_matrix))
    ]
    ds_fc = xr.Dataset(
        data_vars=dict(
...
def get_topo_grid(min_long, max_long, min_lat, max_lat):
    ...
    # elev_file = open("/users/argo/dm_qc/SO_assesment/DMQC-PCM-main/OWC-pcm/matlabow/data/constants/tbase.int", "rb")  # pylint: disable=consider-using-with
    # elev_file = open('L:/users/argo/dm_qc/SO_assesment/DMQC-PCM-main/OWC-pcm/matlabow/data/constants/tbase.int', "rb")  # pylint: disable=consider-using-with
    # elev_file = open(os.path.sep.join([config["CONFIG_DIRECTORY"], "tbase.int"]), "rb")  # pylint: disable=consider-using-with

    # using confg context
    config = get_current_config()
    elevation_file = Path(config["ELEVATION_FILE"])
    with Path.open(elevation_file, "rb") as elev_file:
        if decoder[1] > 4319:
...
def select_ellipses(mat_dict_float, ds, config, map_pv_use=0):
    ...
    float_elev, float_x, float_y = get_topo_grid(
        np.amin(long_float_tbase) - 1,
        np.amax(long_float_tbase) + 1,
        np.amin(lat_float) - 1,
        np.amax(lat_float) + 1,
    )
    # check the dimensions
    # 1️⃣ Extract 1D coordinate vectors from the meshgrid
    x = float_x[0, :] if float_x.ndim == 2 else float_x
    y = float_y[:, 0] if float_y.ndim == 2 else float_y

    # Make sure topo matches the shape
    if float_elev.shape != (len(y), len(x)):
        if float_elev.shape == (len(x), len(y)):
            float_elev = float_elev.T  # transpose if swapped
        else:
            raise ValueError(
                f"Cannot match topo shape {float_elev.shape} with coords x:{len(x)}, y:{len(y)}",
            )

    # Create interpolator
    float_interp = make_safe_rgi(x, y, float_elev)

    # float_z = -float_interp(long_float_tbase, lat_float)[0]
    # float_z = -1 * np.vectorize(float_interp)(long_float_tbase, lat_float)
    points = np.column_stack((lat_float, long_float_tbase))
    float_z = -1 * float_interp(points)
    ...
    grid_elev, grid_x, grid_y = get_topo_grid(
        np.amin(long_vector_tbase) - 1,
        np.amax(long_vector_tbase) + 1,
        np.amin(lat_vector) - 1,
        np.amax(lat_vector) + 1,
    )

    grid_interp = make_safe_rgi(grid_x, grid_y, grid_elev)
    ...
    points = np.column_stack((long_vector_tbase, lat_vector))
    grid_z = -1 * grid_interp(points)
    ...
    for iprof in range(len(mat_dict_float["PROFILE_NO"][0])):  # loop float profiles
        if map_pv_use == 1:
            ellipse = np.sqrt(
                np.power(long_vector_tbase - long_float_tbase[iprof], 2) / np.power(longitude_large * 3, 2)
            ...
            ellipse = np.sqrt(
                np.power(long_vector - long_float[iprof], 2) / np.power(longitude_large * 3, 2)
                + np.power(lat_vector - lat_float[iprof], 2) / np.power(latitude_large * 3, 2),
            )
