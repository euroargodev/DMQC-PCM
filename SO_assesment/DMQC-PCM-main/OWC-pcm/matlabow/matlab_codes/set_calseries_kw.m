...
try
    ...
    breaks = [];
    max_breaks = 0;   % 0 for linear trend and -1 for offset only!
    %calseries = [ones(1,n)];
    calseries = [ones(1,60) 2*ones(1,200-60) 3*ones(1,n-200)];    % example: split the time series at profile 33
    %calseries = [ones(1,160)  2*ones(1,n-160)]; 
    % calseries = [ones(1,33) 0  ones(1,n-33-1)]; % example: ignore profile 34
    %calseries = [ones(1,150) 2*ones(1,n-150)];    % example: split the time series at profile 33
