...
%
% Clive Neil, February 2021: PCM_FILE needs to be dynamically generated
% including the float_WMO and not read from the config file - this is done
% in lines 52 to 54 by looking for a particular float in the PCM_DIRECTORY
%
...
use_pcm = str2double( po_system_configuration.USE_PCM );
%pcm_file = po_system_configuration.PCM_FILE;
pcm_directory = po_system_configuration.PCM_DIRECTORY;
matching_pcm_file = dir(fullfile(pcm_directory, "output_files", "PCM_classes_" + pn_float_name + "_K*.txt"));
pcm_file = fullfile(pcm_directory, "output_files", matching_pcm_file.name);
...
for i = 1 : length( missing_profile_index )
  ...
  PRES = lo_float_source_data.PRES(:,j) ;  
	...
	[ la_bhist_sal, la_bhist_ptmp, la_bhist_pres, la_bhist_lat, la_bhist_long, la_bhist_dates ] = retr_region_ow( la_wmo_numbers, pn_float_name, po_system_configuration, index, PRES, map_p_delta ) ;
        ...
        %n_ref_prof(end +1) = length(la_bhist_long); %KW commented 01/022022
        %n_ref_prof_file =
        %'/home1/homedir5/perso/agarciaj/EARISE/DMQC-PCM/examples/mat_files/3901928/nprof_files/3901928_nprof_argo_noclass_FINAL.mat'; %KW commented 01/022022
        %n_ref_prof_file = [];
...
%if ~isempty(n_ref_prof_file)%KW commented 01/022022
%    save(n_ref_prof_file, 'n_ref_prof')%KW commented 01/022022
%end%KW commented 01/022022
