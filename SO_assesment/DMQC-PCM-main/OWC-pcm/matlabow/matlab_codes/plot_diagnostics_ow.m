...
% Kamila Walicka 28/01/2021 changed in legend in plot 1 and plot 3- set
% legend to skip plot element, change made due to migration from matlab
% 2016b to 2018b
...
hold on
h2 = plot(x,y,'b.');
h3 = plot(coastdata_x,coastdata_y,'k.-');
%legend([h1 h2],{'float','historical points'},'Location','Best')

for i=1:n
  h3=plot(LONG(i),LAT(i),'+');
  set(h3,'color',c(i,:));
...
for j=1:length(ii)
  ...
  xticklabels(ii(j),:)=cellstr(c(1,:)); % modified by matdon 18Dec2018 for Matlab2016b
end
set(gca,'XTickLabel',xticklabels);
legend([h1 h2],'float','historical points')
drawnow
...
hold on
%legend([qq],int2str([PROFILE_NO(jj)]'), 'Location', 'NorthEastOutside')

for i=1:n % plot all remaining profiles
  qqq = plot( PTMP(:,i), SAL(:,i) );
  set(qqq,'color',c(i,:)) ;
end
...
set(gcf,'defaultaxesfontsize',16)
subplot(2,1,1)
h11 = plot(PROFILE_NO, pcond_factor, 'b-');
hold on
h22 = plot(PROFILE_NO, pcond_factor, 'g-');
% plot station by station fit
ok = find(isfinite(sta_mean));
h33 = plot(PROFILE_NO(ok), sta_mean(ok), 'r-');

errorbar(PROFILE_NO, pcond_factor, 2*pcond_factor_err,'b')
errorbar(PROFILE_NO, pcond_factor, pcond_factor_err,'g*-')
h44 = plot(PROFILE_NO(ok), sta_mean(ok), 'r-');

h55 = plot( [0, max(PROFILE_NO)+1], [1,1], 'k-');
...
legend([h11 h22 h33],'2 x cal error','1 x cal error','1-1 profile fit');

subplot(2,1,2)
h1 = plot(PROFILE_NO, avg_Soffset, 'b-');
hold on
h2 = plot(PROFILE_NO, avg_Soffset, 'g-');
% Plot station by station fit
ok = find(isfinite(avg_Staoffset));
h3 = plot(PROFILE_NO(ok), avg_Staoffset(ok), 'r-');

errorbar(PROFILE_NO, avg_Soffset, 2*avg_Soffset_err,'b')
errorbar(PROFILE_NO, avg_Soffset, avg_Soffset_err,'go-')
h4 = plot(PROFILE_NO(ok), avg_Staoffset(ok), 'r-');

axis([ 0, max(PROFILE_NO)+1, min([avg_Soffset-avg_Soffset_err,0])-.02, max([avg_Soffset+avg_Soffset_err,0])+.02 ])
h5 = plot( [0, max(PROFILE_NO)+1], [0,0], 'k-');
set(gca,'FontSize',12)
...
legend([h1 h2 h3],'2 x cal error','1 x cal error','1-1 profile fit');
drawnow
set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.75,8,9.5]);
