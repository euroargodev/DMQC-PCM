...
def BIC_calculation(ds, corr_dist, pcm_features, features_in_ds, z_dim, Nrun=10, NK=20):
    """ Calculation of BIC (Bayesian Information Criteria) for a training dataset.
        ...
        Parameters
        ----------
        ds: xarray bool list(int) dataset
        corr_dist: correlation distance
        pcm_features: dictionary with pcm features {'temperature': z vector}
        features_in_ds: dictionary with the name of features in the model and in the dataset {temperature: theta}
...
def plot_BIC(BIC, NK, float_WMO, plots_directory):
    ...
    plt.title(f'BIC index for: {float_WMO}')

    #print('saving BIC plot')
    plt.savefig(f'{plots_directory}{float_WMO}_BIC.eps')
