        ...
        if cnt == 0:
            ...
            if pres_levels_load > pres_levels:
                ...
                if n_prof_source != n_prof:
                    print(f'XXX skipping box: {ifile} different number profiles {n_prof_source} vs {n_prof}')
                    continue
...
def get_topo_grid(min_long, max_long, min_lat, max_lat):
    ...
    elev_file = open('/users/argo/dm_qc/SO_assesment/DMQC-PCM-main/OWC-pcm/matlabow/data/constants/tbase.int',
                     "rb")  # pylint: disable=consider-using-with
