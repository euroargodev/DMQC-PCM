...
class Plotter:
    ...
    def __init__(self, ds, m, coords_dict=None, float_WMO=None, cmap_name='Accent'):
    ...
    def pie_classes(self, save_fig=False):
    ...
    def vertical_structure(self,
        ...
        fig.suptitle(f'Vertical structure of classes for: {self.float_WMO}', fontsize=12)
...
#        supt = fig.suptitle('$\\bf{Vertical\\ structure\\ of\\ classes}$', fontsize=12)
        supt = fig.suptitle(f'Vertical structure of classes for: {self.float_WMO}', fontsize=12)
        fig_size = fig.get_size_inches()
        #plt.draw()
    ...
    def spatial_distribution(self, proj=ccrs.PlateCarree(), extent='auto', time_slice=0, lonlat_grid=[4,4],
                             float_traj=True, save_fig=False):
        ...
        if float_traj:
            ...
            float_cycles = [int(float(float_source.values[i].lstrip('selected_float_'))) for i in range(len(float_source))]
...
#        title_str = '$\\bf{Spatial\\ ditribution\\ of\\ classes\\ (CTD\\ database)}$'
        title_str = (f'Spatial distribtuion of classes (CTD database) for: {self.float_WMO}')
    ...
    def temporal_distribution(self, time_bins, start_month=0, save_fig=False):
...
#        title_string = r'Percentage of profiles in each class by $\bf{' + time_bins + '}$'
        title_string = f'Percentage of profiles in each class by {time_bins} for: {self.float_WMO}'
    ...
    def float_traj_classes(self, save_fig=False):
        ...
        title_string = f'Float profile classes for: {self.float_WMO}'
    ...
    def float_cycles_prob(self, var_name='PCM_POST', save_fig=False):
        ...
        if 'ROBUSTNESS' in var_name: 
            ...
            ax.set_title(f'Robustness of each float profile for: {self.float_WMO}', fontsize=16)
        else:
            ax.set_ylabel('Posteriors', fontsize=16)
            ax.set_title(f'Probability of a profile to be in its class for: {self.float_WMO}', fontsize=16)
