...
def readSOArgoIndex(argo_index_file_path, header_rows, mode):
    ...
    for index, current_line in enumerate(all_lines):
        ...
        if float_WMO != current_WMO:
            if current_WMO and lat and last_profile_number is not None and float(lat) < -40:
                argo_float_list.append([
                    dac, float_WMO, mode, int(last_profile_number), last_date, lat, long
                ])
            ...
            last_profile_number_re = re.search(r"_\d*.", filename_list[3])
            last_profile_number = last_profile_number_re.group()[1:-1] if last_profile_number_re else None


    argo_index_df = pd.DataFrame(
        data=argo_float_list,
        columns=["DAC", "float_WMO", "mode", "last_profile", "last_date", "lat", "long"])
...
class ARGOINDEXREADER(object):
    ...
    def __init__(self, config_file):
        with open(config_file) as file:
            ...
            self.CONFIG = config["INDEX READER"]

    def run(self):
        argo_index_df = readSOArgoIndex(
...
if __name__ == "__main__":
    ...
    obj = ARGOINDEXREADER("pcm_owc_config.json")
    obj.run()
