...
def get_refdata(float_mat_path, ow_config, map_pv_use=0):
    ...
    wmo_boxes = ow_config['config_directory'] + ow_config['config_wmo_boxes']
    ref_path = ow_config['historical_directory']

    # get float trajectory
    mat_dict_float = sp.io.loadmat(float_mat_path)
    # calculate geographical extent
    plus_box = 10  # degrees
    longitude_large = float(ow_config['mapscale_longitude_large'])
    latitude_large = float(ow_config['mapscale_latitude_large'])
    ...
    WMOboxes_latlon = np.loadtxt(ow_config['WMO_BOXES_LAT_LONG'], skiprows=1)
    ...
    if argo_data and ctd_data:
        # duplicate boxes list
        boxes_list = np.concatenate((boxes_list, boxes_list))
        # start with argo data
        data_folder_prefix = ow_config['HISTORICAL_ARGO_PREFIX']
    elif argo_data:
        data_folder_prefix = ow_config['HISTORICAL_ARGO_PREFIX']
    else:
        data_folder_prefix = ow_config['HISTORICAL_CTD_PREFIX']
    ...
    for ifile in boxes_list:
        ...
        reference_file_path = ref_path + data_folder_prefix + str(int(ifile)) + '.mat'
        try:
            mat_dict_load = sp.io.loadmat(reference_file_path)
        except FileNotFoundError:
            print(f'XXX {reference_file_path} not found')
            continue
        ...
        if cnt == 0:
            ...
            if pres_levels_load > pres_levels:
                ...
                if n_prof_source != n_prof:
                    print(f'XXX skipping box: {ifile} different number of profiles {n_prof_source} vs {n_prof}')
                    continue
    ...
    ds = select_ellipses(mat_dict_float, ds, ow_config, map_pv_use=map_pv_use)
...
def add_floatdata(float_WMO, float_mat_path, ds):
    ...
    source_matrix = [(source_matrix[i] + '_' + str(np.squeeze(mat_dict_float['PROFILE_NO'])[i])) for i in
                     range(len(source_matrix))]
    ds_fc = xr.Dataset(
        data_vars=dict(
...
def get_topo_grid(min_long, max_long, min_lat, max_lat, elevation_file):
    ...
    elev_file = open(elevation_file,  "rb")  # pylint: disable=consider-using-with
...
def select_ellipses(mat_dict_float, ds, ow_config, map_pv_use=0):
    ...
    longitude_large = float(ow_config['mapscale_longitude_large'])
    latitude_large = float(ow_config['mapscale_latitude_large'])
    phi_large = float(ow_config['mapscale_phi_large'])
