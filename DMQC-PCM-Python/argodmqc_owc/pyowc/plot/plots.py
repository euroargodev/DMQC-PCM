...
def trajectory_plot(bath, reef, floats, climatology, float_name, config):
    ...
    coastline = os.path.join(config['CONFIG_DIRECTORY'],
                             "coastline",
    ...
    plt.savefig(plot_loc + "_1_trajectory." + save_format, format=save_format)
...
def theta_sal_plot(sal, theta, map_sal, map_theta, map_errors,
    ...
    save_format = config['FLOAT_PLOTS_FORMAT']
    plot_loc = os.path.join(config['FLOAT_PLOTS_DIRECTORY'], float_name)
    plt.savefig(plot_loc + "_2_" + title + "_theta_sal." + save_format, format=save_format)
    plt.show()
...
def t_s_profile_plot(sal, ptmp, pres, sal_var, theta_levels, tlevels, plevels, float_name, config):
    ...
    save_format = config['FLOAT_PLOTS_FORMAT']
    plot_loc = os.path.join(config['FLOAT_PLOTS_DIRECTORY'], float_name)
    plt.savefig(plot_loc + "_3_salinity_profile." + save_format, format=save_format, bbox_inches='tight')
...
def sal_var_plot(levels, sal, pres, ptmp, map_sal, map_sal_errors,
    ...
    for i in range(levels):
        ...
        save_format = config['FLOAT_PLOTS_FORMAT']
        plot_loc = os.path.join(config['FLOAT_PLOTS_DIRECTORY'], float_name)
        plt.savefig(plot_loc + "_4_salinity_variance_" + str(i + 1) + "." + save_format,
...
def cal_sal_curve_plot(sal, cal_sal, cal_sal_err, sta_sal, sta_sal_err, sta_mean,
    ...
    if np.argwhere(~np.isnan(cal_sal)).__len__() > 0:
        ...
        save_format = config['FLOAT_PLOTS_FORMAT']
        plot_loc = os.path.join(config['FLOAT_PLOTS_DIRECTORY'], float_name)
        plt.savefig(plot_loc + "_5_salinity_curve." + save_format, format=save_format, bbox_inches='tight')
...
def sal_anom_plot(sal, ptmp, profile_no, config, float_name, title='uncalibrated'):
    ...
    for bounds in ast.literal_eval(config['THETA_BOUNDS']):
        ...
        save_format = config['FLOAT_PLOTS_FORMAT']
        plot_loc = os.path.join(config['FLOAT_PLOTS_DIRECTORY'], float_name)
        plt.savefig(plot_loc + "_6_" + title + "_salinity_anomaly_" + str(bounds) + "." + save_format,
                    format=save_format, bbox_inches='tight')
        plt.show()
