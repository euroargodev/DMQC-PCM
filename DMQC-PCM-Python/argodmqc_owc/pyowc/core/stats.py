...
def fit_cond(x, y, n_err, lvcov, *args):
    ...
    for index, nbr in enumerate(pbrk):
        if nbr == -1:
            # offset only
    ...
    if breaks:
        fit_type += 'breaks'
        print('>>>')
        print('>>> WARNING there may be trouble ahead ...')
        print('>>>')
    ...
    if best == 1:
        ...
        for i in range(nloops):
            ...
            if 'breaks' in fit_type:
                print(f'>>> loop {i+1}, nbr {nbr}')
            ...
            if best == 2:
                ...
                print(f'>>> before E A shapes {E.shape[1]} {len(A)}, breaks {len(breaks)}')
                optim = least_squares(nlbpfun, ubrk_g,
                                      method='lm', ftol=tol, max_nfev=max_fun_evals)
                print(f'>>> after  E A shapes {E.shape[1]} {len(A)}, breaks {len(breaks)}')
    ...
    return (xfit, condslope, condslope_err, time_deriv, time_deriv_err,
            sta_mean, sta_rms, ndf, fit_coef, fit_breaks, fit_type)
...
def build_cov(ptmp, coord_float, config):
    ...
    for profile in range(0, ptmp_columns):
        h_cov[profile, :] = covarxy_pv(coord_float[profile], coord_float,
                                       float(config['MAPSCALE_LONGITUDE_SMALL']),
                                       float(config['MAPSCALE_LATITUDE_SMALL']),
                                       float(config['MAPSCALE_PHI_SMALL']),
                                       int(config['MAP_USE_PV']))
