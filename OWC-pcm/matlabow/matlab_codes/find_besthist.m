...
function [ index ] = find_besthist( grid_lat, grid_long, grid_dates, grid_Z, LAT, LONG, DATES, Z, latitude_large, latitude_small, longitude_large, longitude_small, phi_large, phi_small, age, age_large, map_pv_use, max_casts, class_txt )
...
if ~isempty(class_txt)
    ...
    for iprof = 1: length(class_lats)
        iclass_logical = (abs(hist_lat-class_lats(iprof))<1e-3)&(abs(hist_long-class_lons(iprof))<1e-3);
